...
const DefibrillatorEditor: FC<DefibrillatorEditorProps> = ({
	...
	const initialTags =
		data !== null ? data.tags : { emergency: "defibrillator" };
	...
	const [phoneNumber, setPhoneNumber] = useState<string>(
		initialTags.phone ?? initialTags["contact:phone"] ?? "",
	);
	...
	const parseTags: () => Record<string, string> = () => {
		...
		if (location.trim().length > 0)
			tags[`defibrillator:location:${language}`] = location.trim();
	...
	const sendFormData = (event: React.MouseEvent) => {
		...
		if (newAED) {
			...
			getOpenChangesetId(
				...
				.then((changesetId) =>
					addDefibrillatorToOSM(auth, changesetId, newDefibrillatorData),
				)
				.then((newNodeId) => {
					button.classList.remove("is-loading");
					closeSidebar();
					...
					setModalState({
						...initialModalState,
						visible: true,
						type: ModalType.NodeAddedSuccessfully,
						nodeId: newNodeId,
					});
				})
				.catch(handleError);
			...
			getOpenChangesetId(
				...
				.then((changesetId) =>
					editDefibrillatorInOSM(auth, changesetId, defibrillatorData),
				)
				.then((newVersion) => {
					button.classList.remove("is-loading");
					closeSidebar();
			...
			<Card>
				...
				>
					...
					<span className="is-size-5 mr-3 has-text-white-ter has-text-weight-light">
						{t(
							newAED
								? "sidebar.add_defibrillator"
								: "sidebar.edit_defibrillator",
						)}
					</span>
