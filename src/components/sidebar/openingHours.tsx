...
function isCurrentlyOpen(
	...
	try {
		...
		const now = Date.now();
		const minutesOffset =
			new Date(now).getTimezoneOffset() + timezoneOffsetUTCMinutes;
...
export const CurrentlyOpenStatus: FC<OpeningHoursProps> = ({
	...
	const currentlyOpenText = `• ${t(
		isOpen ? "opening_hours.open" : "opening_hours.closed",
	)}`;
	return (
	...
	return (
		<span>
			...
			<CurrentlyOpenStatus
				...
				timezoneOffsetUTCMinutes={timezoneOffsetUTCMinutes}
			/>
		</span>
	);
};
